...
from feemanager.feesetup.feecategories.forms import FeeCategoriesForm
...
def dorms(request):
    return render(request, 'feesetup/feecategories/feecategories.html')
...
def getDorms(request):
    ...
    dorms = Dorms.objects.raw(
        "SELECT DISTINCT v.dorm_code,v.dorm_name, v.max_capacity,v.current_capacity  FROM dorms_dorms v")
...
def searchDorm(request):
    ...
    listsel = []
    dorms =  Dorms.objects.raw(
        "SELECT top 5 dorm_code,dorm_name FROM dorms_dorms v WHERE v.dorm_name like %s or v.dorm_name like %s",
        tuple([query, query]))
