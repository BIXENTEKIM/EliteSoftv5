...
def getCampuses(request):
    listsel = []
    campuses = SchoolCampuses.objects.raw(
        "select campus_code,campus_name,campus_location,campus_incharge,campus_incharge_contacts,"+
        "county_name from campuses_schoolcampuses sc ,students_counties c where sc.campus_county_id= c.county_id")
...
def editCampus(request,id):
    campuses = SchoolCampuses.objects.get(pk=id)
    response_data = {}
...
def searchCounty(request):
    ...
    counties = Counties.objects.raw(
        "SELECT top 5 county_id,county_name FROM students_counties WHERE county_name like %s or county_name like %s",
        tuple([query, query]))
    for obj in counties:
